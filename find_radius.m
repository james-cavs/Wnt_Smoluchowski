function rho_smoldyn=find_radius(N,alpha,dt,DT,deltaN,k1,yi,P_lambda,slo)
lambda=P_lambda/dt;
...
while numrxnrate(step,a,N,DT,alpha,yi,lambda,deltaN,dt,slo) < k1*dt
...
for n = 1:30
    ...
    dif = dif/2;
    a = lo + dif;
    if numrxnrate(step,a,N,DT,alpha,yi,lambda,deltaN,dt,slo) < k1*dt
        lo = a;
    end
...
function k_guess = numrxnrate(step,a,N,DT,alpha,yi,lambda,deltaN,dt,slo)
    ...
    elseif (sindx-2+i < 0)
        ...
        if alpha>1
            if P_lambda<1
                ...
                Kpartial=2*alpha^(2*mu)*pi^(mu+1)*(A)/(gamma(mu)*(alpha^(2*mu)-A));
            elseif P_lambda>=1
                %                Kpartial=2*alpha^(2*mu)*mu*(mu+1)/(alpha^(2*mu)-1);
                Kpartial=2*alpha^(2*mu)*pi^(mu+1)/(gamma(mu)*(alpha^(2*mu)-1));
            end
        elseif alpha<=1&&alpha>0
            
            A=beta^2*besseli(mu+1,beta);
            
            Kpartial=pi^(mu+1)*alpha^mu/(besseli(mu,alpha*beta)*gamma(mu+1))*A;
        elseif alpha==0
            A=beta^(2-mu)*besseli(mu+1,beta);
            
            Kpartial=(2)^mu*(pi)^(mu+1)*A*a^2;
        end
